...
> **`session_worktree_reset_v1`** means the daemon supports worktree ownership transfer: `POST /session/:id/worktree-reset` moves a persisted worktree session's checkout ownership to a fresh replacement session. Restore responses gain three typed 409 classifications alongside it: `worktree_session_superseded` (the session's ownership moved; the body carries `replacementSessionId` for redirect), `worktree_marker_missing` (the checkout marker is absent; retry the restore or reset), and `worktree_reset_interrupted` (a previous transfer crashed mid-flight; retry the reset). See the route section below for the transfer protocol and failure taxonomy.
